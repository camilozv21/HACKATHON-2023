...
def autograph():
    ...
    for combination in combinations:
        ...
        file_name=f"{titles[col]} vs {titles[row]}.jpg"
        ruta='HACKATHON-2023/server/database/data\Plot'

        plt.savefig(ruta+file_name, bbox_inches='tight') 
        plt.close()
...
def plot(df):
  ...
  for pair in all_possible_combinations[130:150]:
    ...
    file_name=f"{df.columns[pair[0]]} vs {df.columns[pair[1]]}"
    plt.title( file_name+
